// Separate test file: nodes/centroid_test.rs. The generated service wires
// it into the crate via `#[cfg(test)] #[path="nodes/centroid_test.rs"] mod
// centroid_test;`. It reaches the node + SDK through `crate::` paths (this is
...
mod tests {
    ...
    // Golden: the area centroid of an axis-aligned square is its center.
    #[test]
    fn test_square_centroid_is_center() {
        ...
        assert!((lon - 1.0).abs() < 1e-9 && (lat - 1.0).abs() < 1e-9, "({},{})", lon, lat);
    }

    // Golden: centroid of a single point is the point itself.
    #[test]
    fn test_point_centroid_is_itself() {
        let ax = test_context();
        let out = centroid(&ax, geom(r#"{"type":"Point","coordinates":[5.5,-3.25]}"#)).unwrap();
        assert_eq!(out.error, "");
        let (lon, lat) = point_coords(&out.geojson);
    ...
    fn test_error_paths() {
        let ax = test_context();
        assert_eq!(centroid(&ax, geom("")).unwrap().error, "EMPTY_INPUT");
        assert_eq!(centroid(&ax, geom("nope")).unwrap().error, "INVALID_GEOJSON");
    }
}
